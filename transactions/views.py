...
def addtransitem(request):
    amount = 0
    inventorytablecolumn = ["ID", 'Item Name',
                            'Selling Price (Per Unit)', 'Expiry Date', "Quantity"]
...
def record(request):
    ...
    table_columns = ["Transaction ID", "Customer Phone No.",
                     "Transaction Date", "Cash", "Card", "Amount"]
    context = {
        'tcol': table_columns,
